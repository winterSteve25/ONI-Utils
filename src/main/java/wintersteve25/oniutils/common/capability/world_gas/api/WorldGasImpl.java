...
public class WorldGasImpl implements IWorldGas {
    ...
    private void spreadVertical(GasStackWrapper gas, BlockPos pos, Element chemicals) {
        ...
        List<GasStackWrapper> upKeys = MiscHelper.getKeysByValue(gasMapUpdated, pos.up());
        List<GasStackWrapper> downKeys = MiscHelper.getKeysByValue(gasMapUpdated, pos.down());
            ...
            if (newPosGas.getType() != currentGas.getType()) {
                ...
                if (!horizontal) {
                    ...
                    if (currentGasDensity > newGasDensity) {
                        ...
                        modifyGasAmount(currentPosition.down(), new GasStackWrapper(new GasStack(currentGas.getType(), amountToSpread)), true);
                    } else {
                        if (!moveUpOrLeft) return;
                        currentGas.shrink(amountToSpread);
                        modifyGasAmount(currentPosition.up(), new GasStackWrapper(new GasStack(currentGas.getType(), amountToSpread)), true);
                    }
                } else {
                    // stuff
                }
    ...
    private void moveGasWithNoObstacleOperation(GasStackWrapper newPosGas, GasStackWrapper currentGas, long amountToSpread, boolean moveUp, boolean horizontal) {
        if (newPosGas == null || newPosGas.isEmpty()) {
            currentGas.shrink(amountToSpread);
            if (moveUp) {
                updateGas(gasMapUpdated.get(currentGas).up(), new GasStackWrapper(new GasStack(currentGas.getType(), amountToSpread)));
            } else {
                updateGas(gasMapUpdated.get(currentGas).down(), new GasStackWrapper(new GasStack(currentGas.getType(), amountToSpread)));
            }
        }
    }
